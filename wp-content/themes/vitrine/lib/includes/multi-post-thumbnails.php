...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			if (null === $this->label || null === $this->id) {
				if (WP_DEBUG) {
					trigger_error(sprintf(__("The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'vitrine'), __CLASS__, __FUNCTION__));
				}
				return;
			}
		...
		public function add_metabox() {
			add_meta_box("{$this->post_type}-{$this->id}", esc_html__( 'Featured Image' , "vitrine"), array($this, 'thumbnail_meta_box'), $this->post_type, $this->context, $this->priority);
		}
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="MultiPostThumbnails.setAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">' . esc_html__( 'Set as %3$s', 'vitrine' ) . '</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			if (version_compare($wp_version, '3.5', '<')) {
				...
				$image_library_url = esc_url( add_query_arg( array( 'context' => $this->id, 'TB_iframe' => 1 ), remove_query_arg( 'TB_iframe', $image_library_url ) ));
				$url_class = "thickbox";
			} else {
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_html__( "Set %s" , 'vitrine' ), $this->label ), $image_library_url, $this->post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html__( "Set %s", 'vitrine' ), $this->label ) );
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf( $format_string, $this->post_type, $this->id, sprintf( esc_html__( "Remove %s", 'vitrine' ), $this->label ), $ajax_nonce );
				}
	...
	if ( is_admin() )
		load_plugin_textdomain( 'vitrine', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
}
