	...
	class WooZoneInsaneImport
	{
		...
		*/
		public function __construct( $is_cron=false )
		{
			...
			add_action('wp_ajax_WooZoneIM_GetScoreHint', array( $this, 'get_score_hint' ), 10, 2);
		...
		*/
		private function printBaseInterface()
		{
			...
			//ob_start;
?>
			<?php echo WooZone_asset_path( 'css', $this->module_folder . 'rangeslider/rangeslider.css', false ); ?>
								...
								?>
									...
									<div class="WooZone-insane-container WooZone-insane-tabs">
										...
										<div class="WooZone-insane-panel-headline">
											<a href="#WooZone-content-search" class="WooZone-insane-button-on"><?php _e('SEARCH FOR PRODUCTS', $this->the_plugin->localizationName);?></a>
														...
														<form id="WooZone-search-products">
															<ul class="WooZone-insane-tab-search-buttons">
																<li>		            						
																	...
																	<input type="text" id="WooZone-search-keyword" name="WooZone-search[keyword]" placeholder="<?php _e('Keyword', $this->the_plugin->localizationName);?>" autocomplete="off" value="<?php echo isset($_REQUEST['keyword']) ? $_REQUEST['keyword'] : '';?>">
																...
																<li id="WooZone-select-on-category">
																	...
																	<input type="text" id="WooZone-search-node" name="WooZone-search[node]" placeholder="<?php _e('Custom BrowseNode', $this->the_plugin->localizationName);?>"/>
																</li-->
																<li>
											...
											<div class="WooZone-insane-import-parameters" id="WooZone-insane-import-parameters">
												...
												<div class="WooZone-insane-import-estimate">
													...
													*/
													...
													<div class="WooZone-insane-import-score-hint"></div>
												</div>
...
<?php } // end demo keys ?>
			...
			<?php
				echo WooZone_asset_path( 'js', $this->module_folder . 'rangeslider/rangeslider.min.js', false );
				echo WooZone_asset_path( 'js', $this->module_folder . 'app.insane_import.js', false );
			?>
...
<?php 
		...
		/*	$html[] = '<div class="WooZone-speedoptimization-legend">';
			$html[] = '<strong>' . ( __('SPEED OPTIMIZATION LEGEND', 'woozone') ) . '</strong>';
			$html[] = '<ul>';
		...
		public function print_score_stats( $stats=array(), $elm_height = 60, $score_hint=false )
		{
			...
			$html = array();
			$html[] = '<h3 style="text-align:center;">' . ( __('Speed Optimization', 'woozone') ) . '</h3>';
			$html[] = '<h4 style="text-align:center;">' . ( __('Estimated score of imported products:', 'woozone') ) . '</h4>';
		...
		public function loadprods_build_html( $prods=array(), $provider='amazon' ) {
			...
			foreach ($prods as $asin => $prod) {
				...
				$html[] = 		'<span class="WooZone-checked-product squaredThree"><input type="checkbox" value="added" name="check" id="squaredThree-'.$asin.'" checked><label for="squaredThree-'.$asin.'"></label></span>';
				...
				if ( isset($prod['SalesRank']) && (int) $prod['SalesRank'] <= 100 ) {
