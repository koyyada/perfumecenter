...
if(function_exists('is_woocommerce')) {
	class epico_WC_Widget_Rating_Filter extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Filter products by rating when viewing product archives and categories.', 'vitrine' );
			$this->widget_id          = 'woocommerce_rating_filter';
			$this->widget_name        = esc_html__( 'Epico WC Average Rating Filter', 'vitrine' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Average Rating', 'vitrine' ),
	...
	add_action( 'widgets_init', 'epico_override_woocommerce_rating_widget', 25 );
	 
	function epico_override_woocommerce_rating_widget() {
	 	// Remove default widget
	  	if ( class_exists( 'WC_Widget_Rating_Filter' ) ) {
	    	unregister_widget( 'WC_Widget_Rating_Filter' );
	 	}
	    register_widget( 'epico_WC_Widget_Rating_Filter' );
	}
