...
function epico_retina_upload_filter( $metadata ){
    ...
    $path     = epico_path_combine($upload['basedir'], $relPath);
    ...
    foreach( $sizes as $key => $size)
    {
        ...
        //Rename the file
        rename(epico_path_combine($path, $sizes[$key]['file']), epico_path_combine($path, $newName));
...
add_filter('wp_generate_attachment_metadata', 'epico_retina_upload_filter' );