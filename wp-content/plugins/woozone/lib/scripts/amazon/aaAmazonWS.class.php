...
class aaAmazonWS
{
	...
	public function accessKey($accessKey = null)
	{
		...
		if ( empty($accessKey) ) {
			...
			throw new InvalidArgumentException('No Access Key has been set');
		}
	...
	public function secretKey($secretKey = null)
	{
		...
		if ( empty($secretKey) ) {
			...
			throw new InvalidArgumentException('No Secret Key has been set');
		}
	...
	public function search($pattern, $nodeId = null)
	{
		...
		{
			throw new InvalidArgumentException('No Category given: Please set it up before');
		}
	 ...
	 */
	public function formatCartItems($cart)
	{
	...
	protected function performXMLRequest($function, $params, $execute=true)
	{
		...
		if ($ret_ == 'invalid') {
			return json_decode(json_encode(array(
				...
				'code' => 1, //error
				'amz_code' => 'woozone:aws.request.dropped',
	...
	protected function aws_signed_request($region, $params, $public_key, $private_key, $associate_tag=NULL, $version='2011-08-01')
	{
		// some paramters
		$method = 'GET';
