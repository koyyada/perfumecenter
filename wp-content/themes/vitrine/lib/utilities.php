                    ...
                    if ( in_array( $term->slug, $options ) ) {
                        if(isset($attr_values[$term->slug]))
                        {
                            ...
                            if($image)
                            {
                                ...
                                echo '<div class="swiper-slide' . esc_attr($selected) .'" data-value="' . esc_attr( $term->slug ) . '"title="' . esc_attr( $term->name ) . '"><image title="' .  esc_attr($term->slug) .'" alt="' .  esc_attr($term->slug) .'" src="' . esc_url($image) . '"></div>';
                            }
                        }
...
if( ! function_exists( 'ep_wc_category_description_position' ) ) {
	function ep_wc_category_description_position() {

		$subTitle = strip_tags(term_description());

	    if (epico_opt('category-description-position') == '0') {
		    ep_wc_category_description_position_action();
        }
        else {
	        ?>
...
if ( ! function_exists( 'ep_wc_category_description_position_action' ) ) {
function ep_wc_category_description_position_action() {
	remove_action( 'woocommerce_archive_description', 'woocommerce_taxonomy_archive_description', 10 );
	remove_action( 'woocommerce_archive_description', 'woocommerce_product_archive_description', 10 );
	add_action( 'woocommerce_before_shop_loop', function () {echo ('<div class="test">');}, 10 );
	add_action( 'woocommerce_before_shop_loop', 'woocommerce_taxonomy_archive_description', 10 );
	add_action( 'woocommerce_before_shop_loop', 'woocommerce_taxonomy_archive_description', 10 );
	add_action( 'woocommerce_before_shop_loop', function () {echo ('</div>');}, 10 );
}
}
