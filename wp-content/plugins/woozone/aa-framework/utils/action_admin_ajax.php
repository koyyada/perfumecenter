	...
	class WooZone_ActionAdminAjax
	{
		...
		*/
		public function __construct( $parent )
		{
			...
			// wizard
			add_action('wp_ajax_WooZoneWizard', array( $this, 'wizard_ajax_request' ), 10, 2);

			// amazon multiple keys
			add_action('wp_ajax_WooZone_AmzMultiKeysAjax', array( $this, 'amzmultikeys_actions' ));
		}
		...
		*/
		...
		public function attributes_clean_duplicate( $retType='die' ) {
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->attrclean_clean_all();
		}
		...
		public function category_slug_clean_duplicate( $retType='die' ) {
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->category_slug_clean_all();
		}
		...
		public function clean_orphaned_amz_meta( $retType='die' ) {    
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->clean_orphaned_amz_meta_all();
		}
		...
		public function delete_zeropriced_products( $retType='die' ) {    
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->delete_zeropriced_products_all();
		}
		...
		public function clean_orphaned_prod_assets( $retType='die' ) {    
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->clean_orphaned_prod_assets_all();
		}
		...
		public function clean_orphaned_prod_assets_wp( $retType='die' ) {    
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->clean_orphaned_prod_assets_all_wp();
		}
		...
		public function fix_product_attributes( $retType='die' ) {    
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->fix_product_attributes_all();
		}
		...
		public function fix_node_childrens( $retType='die' ) {    
			...
			return $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider )->fix_node_childrens();
		}
		...
		public function wizard_ajax_request( $retType='die', $pms=array() )
		{
			...
			//$WooZoneWizard = new WooZoneWizard();
			$WooZoneWizard = WooZoneWizard::getInstance( false );

			if ($action == 'check_amz_keys' ) {

				//$this->the_plugin->main_aff_id()
				$WooZoneWizard->setupAmazonHelper( array(
				...
				$WooZoneWizard->get_ws_object( $this->the_plugin->cur_provider )->check_amazon();
			}
			else if ($action == 'load_step' ) {
				$req = array(
					'step' 					=> isset($_REQUEST['step']) ? $_REQUEST['step'] : '',
				);
				$step_html = $WooZoneWizard->load_step( $req );
		...
		public function amzmultikeys_actions( $retType='die' ) {
			...
			$obj = new WooZoneMultipleAmazonKeys($cfg, $module);
