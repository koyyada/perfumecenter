...
if(function_exists('is_woocommerce')) {
	...
	class epico_WC_Widget_Layered_Nav_Filters extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Shows active layered nav filters so users can see and deactivate them.', 'vitrine' );
			$this->widget_id          = 'woocommerce_layered_nav_filters';
			$this->widget_name        = esc_html__( 'Epico WC Layered Nav Filters', 'vitrine' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Active Filters', 'vitrine' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) || $on_sale || $in_stock || $is_search) {
		        ...
		        if ( get_search_query() ) {
		            ...
		            echo '<li class="chosen search-keyword-active"><a href="' . esc_url( $link ) . '">' . esc_html__( 'Search result for "', 'vitrine' ) . get_search_query() . '"' . '</a></li>';
		        }
				//
				...
				if ( ! empty( $_chosen_attributes ) ) {
					foreach ( $_chosen_attributes as $taxonomy => $data ) {
						foreach ( $data['terms'] as $term_slug ) {
							...
							echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'vitrine' ) . '" href="' . esc_url( $link ) . '">' . esc_html( $term->name ) . '</a></li>';
						}
					}
				}
				...
				if ( $min_price ) {
					...
					echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'vitrine' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Min %s', 'vitrine' ), wc_price( $min_price ) ) . '</a></li>';
				}

				if ( $max_price ) {
					$link = remove_query_arg( 'max_price', $base_link );
					echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'vitrine' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Max %s', 'vitrine' ), wc_price( $max_price ) ) . '</a></li>';
				}
				...
				if ( ! empty( $rating_filter ) ) {
					foreach ( $rating_filter as $rating ) {
						...
						echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'vitrine' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Rated %s out of 5', 'vitrine' ), esc_html( $rating ) ) . '</a></li>';
					}
				}
				...
				if ( $on_sale ) {
					...
					echo '<li class="chosen"><a title="' . esc_attr__( 'Remove filter', 'vitrine' ) . '" href="' . esc_url( $link ) . '">' . esc_html__( 'On Sale', 'vitrine' ) . '</a></li>';
				}

				if ( $in_stock ) {
					$link = remove_query_arg( 'availability', $base_link );
					echo '<li class="chosen"><a title="' . esc_attr__( 'Remove filter', 'vitrine' ) . '" href="' . esc_url( $link ) . '">' . esc_html__( 'In Stock', 'vitrine' ) . '</a></li>';
				}
				//
	...
	add_action( 'widgets_init', 'epico_override_woocommerce_layered_Nav_filter_widget', 25 );
	 
	function epico_override_woocommerce_layered_Nav_filter_widget() {
	 	// Remove default widget
	  	if ( class_exists( 'WC_Widget_Layered_Nav_Filters' ) ) {
	    	unregister_widget( 'WC_Widget_Layered_Nav_Filters' );
	 	}

	    register_widget( 'epico_WC_Widget_Layered_Nav_Filters' );
