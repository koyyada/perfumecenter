...
if(class_exists('WooZoneAmazonHelper') != true) {
	class WooZoneAmazonHelper extends WooZone
	{
		...
		public function __construct( $the_plugin=array(), $params=array() ) 
		{
			...
			// ajax actions
			add_action('wp_ajax_WooZoneCheckAmzKeys', array( $this, 'check_amazon' ), 10, 2);
		...
		public function setupAmazonWS( $params=array() ) {
			...
			$mainoptions = array( 'AccessKeyID', 'SecretAccessKey', 'country', 'main_aff_id' );
		...
		public function stress_test()
		{
			...
			if( $action == 'get_product_data' ){
				...
				if( $asin != "" ) {
					...
					//$rsp = $this->the_plugin->get_ws_object( self::$provider )->api_main_request(array(
		...
		public function is_amazon_valid_response( $response, $operation='' ) {
			$ret = array(
				...
				// woozone custom codes: woozone:aws.init.issue | woozone:aws.request.dropped
				'amz_code' 		=> '',
			);
			...
			if ( (string) $response["$ul"]['Request']['IsValid'] == 'False' ) {
				...
				else if ( isset($response["$ul"]['Request']['Errors']['Error'])
		...
		public function get_product_price( $thisProd, $post_id=null, $pms=array() )
		{
			...
			// list price
			$offers = array(
				'ListPrice'         	=> isset($thisProd['ItemAttributes']['ListPrice']['Amount'])
					? ($thisProd['ItemAttributes']['ListPrice']['Amount'] * $multiply_factor) : '',

				'LowestNewPrice'   		=> isset($thisProd['Offers']['Offer']['OfferListing']['Price']['Amount'])
					? ($thisProd['Offers']['Offer']['OfferListing']['Price']['Amount'] * $multiply_factor) : '',

				'LowestNewSalePrice' 	=> isset($thisProd['Offers']['Offer']['OfferListing']['SalePrice']['Amount'])
					? ($thisProd['Offers']['Offer']['OfferListing']['SalePrice']['Amount'] * $multiply_factor) : false,

				'LowestPrice'       	=> isset($thisProd['VariationSummary']['LowestSalePrice']['Amount'])
					? ($thisProd['VariationSummary']['LowestSalePrice']['Amount'] * $multiply_factor) : '',
			);
			//var_dump('<pre>',$thisProd['ItemAttributes']['ListPrice'], $thisProd['Offers']['Offer']['OfferListing']['Price'] ,'</pre>'); 
			...
			$ret['_price'] = isset($product_meta['product']['sales_price'])
			...
			if ( $do_update ) {
				...
				update_post_meta($post_id, '_price', isset($product_meta['product']['sales_price'])
					&& trim($product_meta['product']['sales_price']) != ""
					? $product_meta['product']['sales_price'] : $product_meta['product']['regular_price']
				);
		...
		public function variation_post( $variation_item, $parent_id, $VariationDimensions )
		{
			...
			if ( isset($variation_post['post_name']) ) {
				unset( $variation_post['post_name'] );
			}
			if ( isset($variation_post['guid']) ) {
				unset( $variation_post['guid'] );
			}

			//var_dump('<pre>-----------------',$variation_post ,'</pre>');
			$variation_post_id = wp_insert_post( $variation_post );
			//$___ = get_post( $variation_post_id, ARRAY_A ); var_dump('<pre>',$___ ,'</pre>');
		...
		public function set_product_meta_options( $retProd, $post_id, $is_variation=true )
		{
			...
			if ( $is_variation == false ) {
				// 2015, october 28 - attributes bug repaired!
				update_post_meta($post_id, '_product_version', $this->the_plugin->get_woocommerce_version());
 ...
 WHERE 1=1 AND a.term_id = %s;";
		...
		public function fix_issues( $retType='die' ) {
			...
			if ( 'reset_sync_stats' == $action ) {
				...
				$optionsList = array(
					'WooZone_sync_prod_notfound', // not used anymore: verified on 2018-march

					'WooZone_sync_cycle_stats',
					'WooZone_sync_last_updated_product',
					'WooZone_sync_last_selected_product',
					'WooZone_sync_first_updated_date',

					'WooZone_sync_currentlist_last_product',
					'WooZone_sync_currentlist_nb_products',
					'WooZone_sync_currentlist_nb_parsed',
					'WooZone_sync_currentlist_prod_trashed',
					'WooZone_sync_currentlist_prod_trash_tries',

					'WooZone_sync_witherror_last_updated_product',
					'WooZone_sync_witherror_last_selected_product',
					'WooZone_sync_witherror_nb_products',
					'WooZone_sync_witherror_nb_parsed',
					'WooZone_sync_witherror_tries',
 ...
 WHERE 1=1 AND a.term_taxonomy_id IN (%s) AND a.taxonomy = '%s' ORDER BY a.term_taxonomy_id ASC;";
		...
		public function get_countries( $key='country' ) {
			...
			if ( 'country' == $key ) {
				...
				return  array(
					...
					'cn' => __('China', $localizationName),
					'in' => __('India', $localizationName),
					'it' => __('Italy', $localizationName), //__('Italia', $localizationName),
					'es' => __('Spain', $localizationName), //__('España', $localizationName),
				...
				return  array(
					...
					'au' => '<a href="https://affiliate-program.amazon.com.au/assoc_credentials/" target="_blank">Australia</a>',
				);
			}
			return array();
		}
		...
		public function api_search_bypages( $pms=array() ) {
			...
			//var_dump('<pre>',$__asinsDebug,'</pre>');
				...
				if (isset($e->faultcode)) { // error occured!
					...
					$response = array('status' => 'invalid', 'msg' => $msg, 'code' => 1, 'amz_code' => strtolower($e->faultcode), 'req_link' => $req_link);
		...
		public function api_make_request( $pms=array() ) {
			...
			switch ( $method ) {
				...
				default:
					$response = array('status' => 'invalid', 'msg' => 'you need to provide a valid method!', 'code' => 1, 'amz_code' => 'woozone:aws.init.issue', 'req_link' => $req_link);
					return array_merge(array('response' => $response), $response);
					//break;
			}
		...
		public function productHasOfferlistingid( $pms=array() ) {
			$pms = array_replace_recursive(array(
				'verify_variations' => false, // true = force verification of variations childs for parent
				'thisProd' 			=> null,
				'post_id' 			=> 0,
			), $pms);
			extract($pms);
			...
			// variable product & it's the parent (not a variation child) - DON'T VERIFY CHILDS
			...
			// variable product & it's the parent (not a variation child) - FORCE CHILDS VERIFICATION
			else {
		...
		public function api_main_request( $pms=array() ) {
			...
			if ( 'WooZoneAmazonHelper::check_amazon' == $from_func ) {
				$this->use_multi_keys = false;	
			}
			...
			do {
				...
				if ( $this->use_multi_keys ) {
					...
					$this->setupAmazonWS( array(
						//'AccessKeyID' 			=> WooZone_generateRandomString(),
						//'SecretAccessKey' 		=> WooZone_generateRandomString(),
				...
				if ( ! is_object($this->aaAmazonWS) ) {
					$msg = $this->the_plugin->wsStatus['msg'];
					$response = array('status' => 'invalid', 'msg' => $msg, 'code' => 1, 'amz_code' => 'woozone:aws.init.issue', 'req_link' => '');
