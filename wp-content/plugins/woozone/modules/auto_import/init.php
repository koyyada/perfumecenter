	...
	class WooZoneAutoImport
	{
		...
		*/
		public function __construct( $is_cron=false )
		{
			...
			$theHelper = $this->the_plugin->get_ws_object( $this->the_plugin->cur_provider );
		...
		*/
		...
		?>
			...
			<!-- current module -->
			<?php echo WooZone_asset_path( 'js', $this->module_folder . 'app.auto_import.js', false ); ?>
			<style type="text/css">
											...
											WooZoneAjaxListTable::getInstance( $this->the_plugin )
												->setup(array(
													...
													'columns'			=> array(
														...
														'imported_created_date'		=> array(
															'th'	=> __('Created Date <br /> Imported Date', $this->the_plugin->localizationName),
															'td'	=> '%imported_created_date%',
															'width' => '120'
														),
														
														'nb_tries'		=> array(
															'th'	=> __('Nb tries', $this->the_plugin->localizationName),
															'title' => __('Nb tries (max = 3)', $this->the_plugin->localizationName),
															'td'	=> '%nb_tries%',
															'align' => 'center',
															'width' => '50'
			...
			foreach ( $search_params_ as $k => $v ) {
				...
				foreach ($v as $key => $val) {
					...
					if ( isset($this->searchParameters["$key"], $this->searchParameters["$key"]['title']) ) {
						...
						$nice_name = $this->the_plugin->category_nice_name( $key );
					}
		...
		public function load_asset( $what='', $print=true ) {
			...
			if ( 'js' == $what ) {
				...
				if( !wp_script_is('jquery-timepicker') ) {
					wp_enqueue_script( 'jquery-timepicker' , WooZone_asset_path( 'js', $this->the_plugin->cfg['paths']['freamwork_dir_url'] . 'js/jquery.timepicker.v1.1.1.min.js', true ), array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'jquery-ui-slider' ), WooZone_asset_version( 'js' ) );
				}

				$asset = WooZone_asset_path( 'js', $this->module_folder . 'app.auto_import.js', false );
			}
			else {
				$asset = $what;
			}
...
<?php
		...
		public function schedule_search_get_params( $pms=array() ) {
			...
			foreach ($this->searchParametersCore as $key => $val) {
				if ( !is_array($requestData['extra_params']) ) {
					$requestData['extra_params'] = array();
				}
		...
		public function auto_save_queue( $pms=array() ) {
			...
			$requestData = array(
				...
				'country' 			 => isset($_REQUEST['country']) ? $_REQUEST['country'] : '',
				'WooZoneInsaneImport' => null,
			);

			if ( empty($requestData['country']) ) {
				$requestData['country'] = $this->the_plugin->amz_settings['country'];
			}
			...
			foreach ($asins as $asin) {
				...
				if ( !isset($asins_db["$asin"]) ) {
					...
					// does NOT exists in wp_posts
						...
						// status: new, failed, done, running
						$product_info = $this->get_product_info( $asin, $WooZoneInsaneImport );
		...
		public function cronjob_queue( $pms, $return='die' ) {
			...
			if ( WooZone()->is_aateam_demo_keys() && ! WooZone()->is_aateam_devserver() ) {
				$ret = array_merge($ret, array(
					'msg' 	=> 'you cannot import products using aateam demo keys as amazon keys.',
				));
				return $ret;
			}
			...
			if (empty($res)) {
				...
				$ret = array_merge($ret, array(
					'status' => 'done',
					'msg' 	=> 'no products found to be (auto) imported - db step.',
				));
				return $ret;
			}
			...
			if (empty($prods)) {
				$ret = array_merge($ret, array(
					'status' => 'done',
					'msg' 	=> 'no products found to be (auto) imported - queue step.',
				));
				return $ret;
			}
			...
			foreach ($prods as $key => $row) {
				...
				$status_msg = implode('<br /><br />', $status_msg);
				$mainstatus_msg[] = "------------------ $key <br /><br />" . $status_msg;
				$status_msg = serialize( $status_msg );
			...
			$mainstatus_msg = implode( '<br /><br /><br />', $mainstatus_msg );
			$ret = array_merge($ret, array(
				'status'            => 'done',
				'msg' 				=> $mainstatus_msg,
			));
		...
		public function cronjob_search( $pms, $return='die' ) {
			...
			$sql = "SELECT a.*, ( isnull(a.run_date) || a.run_date = '0000-00-00 00:00:00' || '$now' >= a.run_date ) as is_run_time FROM " . $table . " as a WHERE 1=1 AND a.publish ='Y' AND ( ( isnull(a.run_date) || a.run_date = '0000-00-00 00:00:00' || '$now' >= a.run_date ) OR ( a.status IN ('new', 'error') AND a.nb_tries < %s ) ) ORDER BY a.nb_tries ASC, a.id ASC LIMIT 0, ".self::$queue_chunk_nb_search.";";
			$sql = $wpdb->prepare( $sql, self::$max_nb_tries );
			$res = $wpdb->get_results( $sql, ARRAY_A);
			...
			if (empty($res)) {
				...
				$ret = array_merge($ret, array(
					'status' => 'done',
					'msg' 	=> 'no searches found to be run at this moment - db step.',
				));
				return $ret;
			}
			$prods = $res;
			...
			foreach ($prods as $key => $row) {
				...
				if ( !$search_stat ) { // INVALID
					...
					// run search
					$WooZoneInsaneImport->setupAmazonHelper( array(
						'country' 				=> $searchParams['country'],
						'main_aff_id'			=> $searchParams['main_aff_id'],
					));

					// DEBUG
					//var_dump('<pre>', $WooZoneInsaneImport->get_ws_object( $this->the_plugin->cur_provider ), '</pre>');
					//continue 1;
					...
					$this->auto_save_queue(array(
						...
						'country' 			 => $row['country'],
						'WooZoneInsaneImport' => $WooZoneInsaneImport,
					));
				}
				...
				*/
				...
				$sql_upd = $wpdb->prepare($sql_upd,
					$status_s,
					$status_msg,
					$nb_tries, //(int) ($row['nb_tries'] + 1), //changed in 2018-mar-12
					date("Y-m-d H:i:s"),
					'%Y-%m-%d %H:%i:%s',
		...
		private function parse_from( $from ) {
			$_from = explode('#', $from);
			$id = 0; $from = '';
		...
		private function get_import_params_search( $rows ) {
			...
			foreach ($rows as $key => $row) {
				$from = $this->parse_from($row['from_op']);
		...
		private function get_product_info( $asin, $WooZoneInsaneImport=null ) {

			$provider = 'amazon';

			if ( ! is_object($WooZoneInsaneImport) ) {
				// Initialize the WooZoneInsaneImport class
				require_once( $this->the_plugin->cfg['paths']['plugin_dir_path'] . '/modules/insane_import/init.php' );
				$WooZoneInsaneImport = new WooZoneInsaneImport(true);
				//$WooZoneInsaneImport = WooZoneInsaneImport::getInstance();
			...
			$__cache = $WooZoneInsaneImport->getTheCache( $__cachePms );
			$__cachePage = ( $__cache !== false ? $__cache : array() );

			// cache is found!
			$product = array();
			if ( !empty($__cachePage)
				&& $WooZoneInsaneImport->get_ws_object( $provider )->is_valid_product_data($__cachePage) ) {
