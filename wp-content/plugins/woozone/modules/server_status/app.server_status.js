...
WooZonePriceUpdateMonitor = (function ($) {
    "use strict";
	...
	function syncProduct( row, id )
	{
		...
		jQuery.post(ajaxurl, {
			...
			if( response.status == 'valid' ){
				...
				row.find(".WooZone-data-price").html( html_price );
				
				row.find(".WooZone-data-last_sync_date").html( response.data.last_sync_date );
			}
	...
	function stress_test_step1(that)
	{
		...
		// get the ASIN code 
		var ASIN = $("#WooZone-test-ASIN").val();
		
		// show the timeline 
		var timeline = $(".WooZone-test-timeline");
		timeline.show();
		
		// show log container
		var logs_container = $(".WooZone-table.WooZone-logs");
		logs_container.show();
		...
		var status_box_step = $("#stepid-step1").find(".WooZone-step-status"); 
		
		jQuery.post(ajaxurl, {
			'action' 		: 'WooZoneStressTest',
			'ASIN'			: ASIN,
			...
			logs_container.find("#logbox-step1 .WooZone-log-details").val( JSON.stringify( response.log ) );
			
			// show the status into timeline
			status_box_step.removeClass('WooZone-loading-inprogress');
				
			if( response.status == 'valid' ){
				status_box_step.addClass('WooZone-loading-success');
				...
				status_box_step.addClass('WooZone-loading-error');
		...
		.error(function(response) { 
			...
			logs_container.find("#logbox-step2 .WooZone-log-details").val( messge );
			
			status_box_step.removeClass('WooZone-loading-inprogress').addClass('WooZone-loading-error');
			status_box_step.html("error");
		});
	}
	...
	function stress_test_step3( timeline, logs_container )
	{
		...
		var status_box_step = $("#stepid-step3").find(".WooZone-step-status");
		status_box_step.addClass('WooZone-loading-inprogress');
		...
		jQuery.post(ajaxurl, {
			...
			logs_container.find("#logbox-step3 .WooZone-log-details").val( JSON.stringify( response.log ) );
		...
		.error(function(response) { 
			...
			var messge = "Please contact your server administrator and ask about this error: " + response.status + ": " + response.statusText;
			logs_container.find("#logbox-step3 .WooZone-log-details").val( messge );
	...
	function stress_test_step2( timeline, logs_container )
	{
		...
		var status_box_step = $("#stepid-step2").find(".WooZone-step-status");
		status_box_step.addClass('WooZone-loading-inprogress');
		...
		jQuery.post(ajaxurl, {
			...
			// save the log
			logs_container.find("#logbox-step2").css('opacity', 1);
			logs_container.find("#logbox-step2 .WooZone-log-details").val( JSON.stringify( response.log ) );
		...
		.error(function(response) { 
			...
			var messge = "Please contact your server administrator and ask about this error: " + response.status + ": " + response.statusText;
			logs_container.find("#logbox-step1 .WooZone-log-details").val( messge );
	...
	{
		jQuery.post(ajaxurl, {
			'action' 		: 'WooZoneServerStatusRequest',
			'sub_action'	: action,
			'debug_level'	: debug_level
		}, function(response) {
	...
	function triggers()
	{
		maincontainer.on('click', 'input.WooZone-sync_now', function(e){
			e.preventDefault();
		...
		maincontainer.on('click', 'a.WooZoneStressTest', function(e){
			e.preventDefault();
		...
		maincontainer.on('click', '.WooZone-log-title a', function(e){
			e.preventDefault();
			
			var that = $(this),
				parent = that.parent('div');
			
			parent.next(".WooZone-log-details").show();
		});
		
		maincontainer.on('click', '.WooZone-export-logs', function(e){
			e.preventDefault();
			
			export_logs( $(".WooZone-panel-content > .WooZone-table" ) );
		});
		
		$(".WooZone-loading-ajax-details").each( function(){
			var that = $(this),
		...
		maincontainer.on('click', '.WooZone-check-integrity-container > a', function(e){
			e.preventDefault();
		...
		maincontainer.on('click', 'a.WooZoneEnableAjaxDebug', function(e){
			e.preventDefault();
			...
			jQuery.post(ajaxurl, {
				'action': 'WooZoneServerStatusRequest',
				'sub_action': 'WooZoneEnableAjaxDebug'
			}, function(response) {
				if( response.status == 'valid' ) {
	...
	function ajax_operation( that, action ) {
		//return false;
		jQuery.post(ajaxurl, {
			'action' 		: 'WooZoneServerStatusOperation',
			'sub_action'	: action,
			'debug_level'	: debug_level
		}, function(response) {
			if( response.status == 'valid' ) {
				var $resp = that.parent().find('.WooZone-response');
